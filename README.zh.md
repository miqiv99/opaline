...
- “认真记记”开始走类 Obsidian 信息架构：左侧快捷栏、文件栏、主编辑器、右侧检查器、左右栏折叠和小型图谱预览。

仍然粗糙的地方：

- 更改工作区位置后，还没有迁移原工作区文件的流程。
- 图谱目前只是右侧小预览，还不是完整图谱工作区。
...
4. 把右侧小图谱升级成真正的图谱视图，支持过滤、点击跳转和定位当前笔记。
5. 继续强化 HTML 原生交互块，让双栏、旁注、标注、嵌入、折叠块像界面能力，而不是隐藏标签。
6. 继续用可靠 parser 和测试加固 Opaline HTML Profile。
7. 深化 SQLite FTS 搜索、元数据扫描、反链和断链修复。
8. 增加带来源的本地笔记 AI 检索，先展示命中的笔记和片段，再生成回答。
9. 增加明确的 AI 笔记操作：总结、提取标签、建议链接、拆分长笔记、生成大纲。
10. 后续再做静态发布和公开笔记的读者选中概念搜索。
