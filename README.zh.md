...
- ✓ 轻量文档样式系统：正文、标题、标注块和代码块可设置字体族、字号和颜色
- ✓ 统一编辑命令注册表：工具栏、右键菜单、slash 命令、命令面板和未来 AI 编辑流程共用同一套命令
- ✓ 编辑器 slash 命令菜单：支持标题、列表、富块、媒体、数学公式、图表、布局和折叠块
- ✓ 编辑器命令面板：可从工具栏打开，也可用 `Ctrl+Shift+P` 打开
- ◐ AI 编辑工具协议：包含命令描述、JSON schema 校验、权限、预览元数据、确认门槛、历史快照钩子、风险元数据和回滚钩子
- ◐ SQLite 元数据、搜索方向、标签、标题、本篇关联、谁提到本篇和断链检测
- ✓ 工作区诊断：检查 HTML Profile 问题、断链、附件引用、重复 ID 和 SQLite 索引漂移
- ✓ 显式重建 SQLite 索引：刷新派生元数据，不修改 HTML 笔记文件
- ✓ 文件级、标题级、块级、概念级关系图
- ✓ 本地笔记历史快照
- ✓ 工作区备份 / 导出：备份前检查笔记、资源、设置和本地历史
- ✓ 安全的工作区迁移预检与复制流程：迁移后保留旧工作区
...
- 工作区迁移目前采用保守的复制流程；后续还可以继续补恢复、清理旧目录等辅助工具。
- 部分文件操作和历史版本控制还需要打磨。
...
- 工作区备份会包含 `notes/`、`assets/`、`.opaline/settings.json`，以及存在时的 `.opaline/history/`；默认不包含 `.opaline/cache/` 和可重建的 SQLite 索引。
- 工作区迁移会把工作区复制到新位置、校验副本、让 Opaline 指向新位置，并保留旧工作区不动。

本地编辑不需要云账号。只有在用户配置 AI provider、抓取模型列表、运行扩展组件或脚本、打开外部链接时，才可能发生网络访问。
