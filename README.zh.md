...
- 支持通过“今天”创建日记。
- “随便记记”会把用户输入先写入当天 HTML 日记，再让 AI 继续回应。
...
- “认真记记”开始走类 Obsidian 信息架构：左侧快捷栏、文件栏、主编辑器、右侧检查器、左右栏折叠、小型图谱预览和完整图谱视图；完整图谱已经有当前笔记邻域模式和边详情。

仍然粗糙的地方：

- 更改工作区位置后，还没有迁移原工作区文件的流程。
- 从选中文字创建概念级关系的普通用户交互还没做，当前只是数据模型和图谱先支持。
- 文件操作还不完整，重命名、删除、移动、系统资源管理器显示、历史版本等需要继续补。
...
- 初始页只有两个大入口：“认真记记”和“随便记记”。
- “认真记记”进入类 Obsidian 的知识工作台，适合明确知道自己要记录什么的人。
- “随便记记”进入 Today 对话入口，用户可以随手吐槽、复盘或提问；内容先写入当天 HTML 日记，再由 AI 接着回应。
...
1. 继续打磨“认真记记”的类 Obsidian 工作区，让文件栏、侧栏、编辑器和右侧信息区更统一。
2. 补工作区迁移：用户更改位置时，可选择复制或移动原工作区文件。
3. 扩展文件右键菜单：重命名、删除、移动、在系统资源管理器中显示、复制相对/绝对路径、打开历史版本。
4. 继续改进图谱视图：在当前邻域和边详情基础上，补更好的布局、边到具体目标的导航和保存过滤预设。
5. 给普通用户补上从编辑器右键创建概念级关系，以及跨笔记选择标题/块目标的交互。
6. 继续强化 HTML 原生交互块，让双栏、旁注、标注、嵌入、折叠块像界面能力，而不是隐藏标签。
7. 继续用可靠 parser 和测试加固 Opaline HTML Profile。
8. 深化 SQLite FTS 搜索、元数据扫描、反链和断链修复。
9. 增加带来源的本地笔记 AI 检索，先展示命中的笔记和片段，再生成回答。
10. 增加明确的 AI 笔记操作：总结、提取标签、建议链接、拆分长笔记、生成大纲。
11. 后续再做静态发布和公开笔记的读者选中概念搜索。
