...
Keep the updater entry as an object, even when changing release channels:
...
- the macOS app update archive, such as `Opaline.app.tar.gz`
- the matching `.sig` files generated next to those update artifacts
- a `latest.json` file that points at the real asset URLs and embeds the text content of each `.sig` file

The v0.1.2 manifest used for the current GitHub Release is tracked at `release-assets/v0.1.2/latest.json`.
...
```json
{
  ...
  "platforms": {
    ...
    "darwin-aarch64": {
      "signature": "PASTE_CONTENTS_OF_APP_TAR_GZ_SIG_HERE",
      "url": "https://github.com/miqiv99/opaline/releases/download/v0.1.2/Opaline.app.tar.gz"
    }
  }
}
```
